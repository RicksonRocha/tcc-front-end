...
export default function MyTeamForm({ teamId }) {
  ...
  const sortedStudents = students.filter(s => s.name).map(s => ({ userId: s.id, userName: s.name }));
  ...
  const availableTeachers = teachers.filter((teacher) => {
    ...
    return pref && pref.availableForAdvising === 'Sim';
  });

  const sortedTeachers = availableTeachers.map(t => ({ userId: t.id, userName: t.name })).sort((a, b) => a.userName.localeCompare(b.userName));
  ...
  const onSubmit = async (data) => {
    let membersList = data.members.map((m) => ({
      userId: m.userId ?? user.id,
      userName: m.userName ?? m,
    }));

    if (!membersList.some((m) => m.userId === user.id)) {
      membersList.push({ userId: user.id, userName: user.name });
    }
            ...
            <Grid xs={12}>
              <Controller name="members" control={control} render={({ field }) => (
                <Autocomplete
                  ...
                  isOptionEqualToValue={(option, value) => (option.userId ?? option) === (value.userId ?? value)}
                  value={field.value}
                  onChange={(e, newValue) => field.onChange(newValue)}
                  renderTags={(value, getTagProps) =>
                    value.map((option, index) => (
                      <Chip key={option.userId ?? option} label={option.userName ?? option} {...getTagProps({ index })} />
                    ))
                  }
                  renderInput={(params) => (
                    <TextField {...params} label="Nomes dos Integrantes" error={!!errors.members} helperText={errors.members?.message} />
                  )}
                />
              )} />
            </Grid>
