...
export const preferenciasUserAluno = createAsyncThunk(
  ...
  async ({ turno, linguagemProgramacao, bancoDeDados, nivelDeExperiencia, habilidadesPessoais, temasDeInteresse, disponibilidade, modalidadeTrabalho}, { rejectWithValue }) => {
    try {
      await axios.post(`${backendURL}/auth/preferencias-aluno`, { turno, linguagemProgramacao, bancoDeDados, nivelDeExperiencia, habilidadesPessoais, temasDeInteresse, disponibilidade, modalidadeTrabalho}, config);
    } catch (error) {
      if (error.response.data.error) {
        return rejectWithValue(error.response.data.error);
      }
